...
class OpenBCICyton(object):
    ...
    def __init__(self, port=None, baud=115200, filter_data=True, scaled_output=True,
                 daisy=False, aux=False, impedance=False, log=False, timeout=None):
        ...
        self.tem_data_buffer = []
        self.QUE = Queue()
        self.stpEv = Event()    #退出时子线程告知主线程确实结束，避免报错
        self.stpEv.clear()
    ...
    def read_data(self):    #返回channels x n 数据
        data = self.QUE.get()
    ...
    def callback(self,sample):
        self.tem_data_buffer.append(sample.channel_data)
        if len(self.tem_data_buffer) == 100: #约400ms
            self.QUE.put(self.tem_data_buffer)
            self.tem_data_buffer = []
